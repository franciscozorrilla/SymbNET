...
```bash
$ for i in {1..5}; do 
echo "Running simulation $i out of 5 ... "; 
smetana --flavor ucsd -o sim_${i} -v -d --molweight --zeros *.xml;
done
```
...
$ while read file;do 
 com=$(dirname $file|sed 's/$//g');sim=$(basename $file|sed 's/_detailed.tsv//g'|sed 's/sim_//g');cat $file|grep -v smetana|sed "s/^all/$com/g"|sed "s/^/$sim\t/g" >> smet_all.tsv;
 done< <(find models/$COMM/ -name "*.tsv")
```

Make sure that the names of your newly created models match exactly with the pre-computed ones to ensure that the plotting script runs smoothly. Hint: use the `cp`, `mv`, and/or `rm` commands.
