...
If you successfully generated detailed predictions for your community, summarize them into a file named `${COMM}_5_0_global.tsv`, and move into appropriate subdirectory `/simulations/global/$COMM` e.g.

```bash
$ echo -e "community\tmedium\tsize\tmip\tmro" >> ${COMM}_5_0_global.tsv
$ while read file;do 
cat $file|grep -v comm|sed "s/all/$COMM/g";
done< <(find . -name "sim*global.tsv") >> ${COMM}_5_0_global.tsv
$ cp ${COMM}_5_0_global.tsv ../../data/cooccurrence/simulation/mip_mro/${COMM}
```
